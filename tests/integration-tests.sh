...
RES=$(peggycli query nameservice current-valset $QUERY_FLAGS)
...
peggycli tx nameservice valset-request $TX_FLAGS > /dev/null
BLOCK=$(peggycli status $QUERY_FLAGS | jq .sync_info.latest_block_height -r)
...
RES=$(peggycli query nameservice valset-request $NONCE $QUERY_FLAGS)
