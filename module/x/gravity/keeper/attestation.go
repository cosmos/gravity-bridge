...
// FIXME: change due to non-determinism
func (k Keeper) GetAttestationMapping(ctx sdk.Context) (out map[uint64][]types.Attestation) {
	...
	k.IterateAttestation(ctx, func(attestation types.Attestation) bool {
		...
		if val, ok := out[claim.GetEventNonce()]; !ok {
			out[claim.GetEventNonce()] = []types.Attestation{attestation}
		} else {
			out[claim.GetEventNonce()] = append(val, attestation)
		}
		return false
	})
	return
...
func (k Keeper) GetLastEventNonceByValidator(ctx sdk.Context, validator sdk.ValAddress) uint64 {
	...
	if len(bytes) == 0 {
		...
		for nonce, atts := range attmap {
			for attestation := range atts {
				if atts[attestation].Observed && nonce < lowestObserved {
