...
func lastBatchRequestsByAddressHandler(cliCtx context.CLIContext, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		vars := mux.Vars(r)
		operatorAddr := vars[bech32ValidatorAddress]
		...
		if len(res) == 0 {
			rest.WriteErrorResponse(w, http.StatusNotFound, "no pending batchs found")
			return
		}
...
func lastBatchRequestsHandler(cliCtx context.CLIContext, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		res, height, err := cliCtx.Query(fmt.Sprintf("custom/%s/allBatches", storeName))
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}
		if len(res) == 0 {
			rest.WriteErrorResponse(w, http.StatusNotFound, "no batches found")
			return
		}
...
func lastBatchRequestByNonceHandler(cliCtx context.CLIContext, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		vars := mux.Vars(r)
		nonce := vars[nonce]
		...
		if len(res) == 0 {
			rest.WriteErrorResponse(w, http.StatusNotFound, "no batch found by that nonce")
			return
		}
...
func lastSignedBatchesHandler(cliCtx context.CLIContext, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(res) == 0 {
			rest.WriteErrorResponse(w, http.StatusNotFound, "no inflight batches found")
			return
		}
